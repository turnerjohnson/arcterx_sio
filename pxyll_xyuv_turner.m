function [c1,h1,c2,h2] = pxyll_xyuv_turner(mapstruct,var1str,var2str,whichuv,iz,itime,cont1,cont2,cont2h,nskip)
...
elseif strcmp(whichuv,'obs')
   u=squeeze(mapstruct.udopalong(iz,:,:,itime));
   v=squeeze(mapstruct.udopacross(iz,:,:,itime));
   utitl='Observed velocity';
   uscale=uscl;
   ulab='1 m/s';
elseif strcmp(whichuv,'ageo')
   u=squeeze(mapstruct.ualong(iz,:,:,itime));
   v=squeeze(mapstruct.uacross(iz,:,:,itime));
